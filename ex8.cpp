...
int main()
{
  ...
  const double x_scale = double(xlength / numbers.size());
  const double y_scale = double(ylength) / (*max_element(numbers.begin(), numbers.end()));
  ostringstream osx, osy;
  osx << "Height, cm " << "(5 cm == " << (int)x_scale << " pixels)";
  osy << "Number of people " << "(1 person == " << (int)y_scale << " pixels)";
